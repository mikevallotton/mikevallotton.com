...
export default function AiSearchAndGeoPage() {
  return (
    <TopicPage
      ...
      sources={sources}
      afterSections={<><section aria-labelledby="page-faq-title" className="border-t border-library-parchment py-14 md:py-20"><div className="grid gap-8 md:grid-cols-[15rem_1fr] md:gap-10"><div><p className="text-xs font-semibold uppercase tracking-[0.18em] text-library-walnut">Page-level perspective</p><h2 id="page-faq-title" className="mt-2 text-2xl font-semibold">The bigger questions</h2></div><FaqList items={pageFaqs} /></div></section><section className="border-t border-library-parchment py-14 md:py-20"><div className="max-w-3xl"><h2 className="text-3xl font-semibold leading-tight md:text-4xl">Become a Source AI Can Trust</h2><p className="mt-5 text-lg font-medium leading-relaxed text-library-ink md:text-xl">The future of search belongs to organizations that produce accurate information, structure it well, and make it easy to retrieve.</p><p className="mt-4 max-w-2xl leading-relaxed text-library-muted">Attracting visitors still matters, but it is no longer the only goal. Build a website that functions as a credible knowledge source, and evaluate success by influence and business outcomes as well as traffic.</p><blockquote className="mt-8 topic-accent-border border-l-2 pl-5 font-serif text-xl font-medium leading-relaxed text-library-ink md:text-2xl">In AI search, the most useful source often matters more than the loudest page.</blockquote></div></section></>}
